class EnquiriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @enquiry.save
        begin
          Notifications.enquiry(@enquiry).deliver
          format.html { redirect_to(@enquiry, :notice => 'Thank you for your enquiry.  An email will be sent to you shortly to confirm this enquiry.') }
          format.xml  { render :xml => @enquiry, :status => :created, :location => @enquiry }
        rescue
          format.html { redirect_to(@enquiry, :notice => 'Cannot send email') }
          format.xml  { render :xml => @enquiry, :status => :created, :location => @enquiry }
        end
